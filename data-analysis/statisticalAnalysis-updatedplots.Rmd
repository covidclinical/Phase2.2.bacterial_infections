...
p1= ggplot(dat_combined %>% filter(site=="BCH")) + geom_col(aes(x=timeToGroup,y=bacterial))+geom_line(aes(x=timeToGroup,y=pred))+ggtitle("BCH")+labs(x="",y="bacterial infection") + theme_bw()
p2= ggplot(dat_combined %>% filter(site=="UMICH")) + geom_col(aes(x=timeToGroup,y=bacterial))+geom_line(aes(x=timeToGroup,y=pred))+ggtitle("UMICH")+labs(x="",y="bacterial infection") + theme_bw()
p3= ggplot(dat_combined %>% filter(site=="HCUV")) + geom_col(aes(x=timeToGroup,y=bacterial))+geom_line(aes(x=timeToGroup,y=pred))+ggtitle("HCUV")+labs(x="",y="bacterial infection") + theme_bw()
p4= ggplot(dat_combined %>% filter(site=="H12O")) + geom_col(aes(x=timeToGroup,y=bacterial))+geom_line(aes(x=timeToGroup,y=pred))+ggtitle("H12O")+labs(x="",y="bacterial infection") + theme_bw()
p5= ggplot(dat_combined %>% filter(site=="CCMC")) + geom_col(aes(x=timeToGroup,y=bacterial))+geom_line(aes(x=timeToGroup,y=pred))+ggtitle("CCMC")+labs(x="",y="bacterial infection") + theme_bw()
p6= ggplot(dat_combined %>% filter(site=="GOSH")) + geom_col(aes(x=timeToGroup,y=bacterial))+geom_line(aes(x=timeToGroup,y=pred))+ggtitle("GOSH")+labs(x="",y="bacterial infection") + theme_bw()
p7= ggplot(dat_combined %>% filter(site=="UPMC")) + geom_col(aes(x=timeToGroup,y=bacterial))+geom_line(aes(x=timeToGroup,y=pred))+ggtitle("UPMC")+labs(x="",y="bacterial infection") + theme_bw()
p8= ggplot(dat_combined %>% filter(site=="CCHMC")) + geom_col(aes(x=timeToGroup,y=bacterial))+geom_line(aes(x=timeToGroup,y=pred))+ggtitle("CCHMC")+labs(x="",y="bacterial infection") + theme_bw()
...
panelC_data$vline_label <- ifelse( panelC_data$date %in% as.Date(c("2020-01-01","2021-01-01","2022-09-01","2022-12-01")), "Change Point",
                             ifelse( panelC_data$date %in% as.Date(c("2020-03-01", "2021-03-01")), "Full NPI Period", ""))
...
panelC_data$site <- factor(panelC_data$site, levels = c("BCH","CCHMC", "CCMC", "UMICH", "UPMC","H12O", "HCUV", "GOSH"))


# Convert date to Date format if not already in that format
panelC_data$date <- as.Date(panelC_data$date)

panelC <- ggplot(data=panelC_data, aes(x=date, y=pred, colour=site, linetype=site)) + 
  geom_line() +
  ylab("predicted bacterial infection") +
  
  # Add shaded area between the dates 2020-03-01 and 2022-03-01
  geom_rect(aes(xmin = as.Date("2020-03-01"), xmax = as.Date("2021-03-01"), 
                ymin = -Inf, ymax = Inf, fill = "Full NPI period"), 
            alpha = 0.01, color = NA, show.legend = TRUE) +  # Adjust alpha for transparency
  
    # Define color for the shaded area in the legend
  scale_fill_manual(name = "Event", values = c("Full NPI period" = "#D3D3D3")) +  # Adjust fill color
  
    # Add vertical dotted lines at specific dates
  geom_vline(xintercept = as.Date(c("2020-01-01", "2021-01-01", "2022-09-01", "2022-12-01")), 
             linetype = "dotted", size = .5, colour = "black") +
  
  
  # Customize color and linetype for sites
  scale_color_manual(name = "Site", values = c(
    "BCH" = "#1F77B4", 
    "CCHMC" = "#FF7F0E", 
    "CCMC" = "#2CA02C",
...
tiff(filename = "../../panelFigure_respiratory.tiff",
width = 2400,
height = 1800,
units = "px",
res = 300)
