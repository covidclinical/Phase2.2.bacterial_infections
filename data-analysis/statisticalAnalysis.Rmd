...
panelC_data$vline_label <- ifelse( panelC_data$date %in% as.Date(c("2020-01-01","2021-01-01","2022-09-01","2022-12-01")), "Predicted",
                             ifelse( panelC_data$date %in% as.Date(c("2020-03-01", "2021-03-01")), "NPIs", ""))
...
panelC_data$significant <- ifelse( panelC_data$date %in% as.Date(c("2020-07-01","2022-01-01")), "significant", "")
...
panelC <- ggplot(data=panelC_data,aes(x=date,y=pred,colour=site)) + 
  ...
  scale_linetype_manual(name = "Events", values = c("Predicted" = "dashed", "NPIs" = "dotted")) +
  guides(color = guide_legend(title = "Site"),
         linetype = guide_legend(title = "Events"),
         color = guide_legend(title = "Site"),
         linetype = guide_legend(title = "Events")) +
  theme(legend.key = element_rect(fill = "white")) +  geom_point(data = panelC_data[panelC_data$significant =="significant", ],aes(x=date, y=40),shape = "*", size=5, color ="black", show.legend = FALSE)
