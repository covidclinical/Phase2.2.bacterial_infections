...
Create the barplots
```{r}
bacterial_counts_bySite$site <- factor(bacterial_counts_bySite$site, levels = c("BCH","CCHMC", "CCMC", "MICHIGAN", "UPitt","H12O", "HCUV", "GOSH"))
...
valuesToPlot <- valuesToPlot %>%
  dplyr::mutate( category_name = ifelse( category_name == "IBI_extensive", "Invasive Bacterial Infection", category_name )) %>%
...
png(filename = "../../figure2_linePlot_HighCategory_byHospital.png",
    width    = 800, 
    height   = 600 )
linePlot
dev.off()

ggsave(filename = "../../figure2_linePlot_HighCategory_byHospital_700dpi.png",
       plot = linePlot, 
       width = 10, 
       #height = 450, 
       dpi=700)
...
```{r, eval=FALSE}
valuesToPlot$site <- factor(valuesToPlot$site, levels = c("BCH","CCHMC","CCMC", "MICHIGAN","UPitt", "H12O", "HCUV", "GOSH"))
...
violin_output <- ggplot2::ggplot(valuesToPlot, aes(x=as.factor(site), y=count, fill=country)) +
  ...
  ggplot2::facet_wrap( ~category_name, scales = "free",nrow = 3)+
