...
The *Visits* inclusion criteria includes:
    ...
    -   Comprising at least one ICD code during hospitalization
...
#obfuscation = 3
dateFormat <- "%Y-%m-%d"
data_update_date <- "2023-07-01"
dir.output <- "./bacterialInfection_outputs/"
...
icd10Codes <- read.csv("../public-data/ICD_bacterial5.csv", header = TRUE, colClasses = "character") %>%
   dplyr::mutate( concept_code = gsub("\\.", "",ICD10_Code)) %>%
   dplyr::select( disorder_group = syndrome, concept_code, description,
                  syndrome, IBI_simple, IBI_extensive, Respiratory, Skin, bacterial_syndrome )

icd9Codes <- read.csv("../public-data/ICD9_bacterialMap.csv", header = TRUE, colClasses = "character") %>%
   dplyr::mutate( concept_code = gsub("\\.", "",ICD9_Code)) %>%
   dplyr::select( disorder_group = Syndrome, concept_code, description = Description,
                  IBI_simple, IBI_extensive, Respiratory, Skin, bacterial_syndrome )

icdCodes <- rbind( icd10Codes, icd9Codes)
```
...
# in the case of valencia the have he age at diagnosis already so we just use the column from the observation file
obs_raw_age <- obs_raw_age %>%
    dplyr::mutate( age_time_diagnosis = ageAtAdmission) %>%
    dplyr::select( -ageAtAdmission )
...
all_codes_desc <- all_codes %>% 
  ...
  dplyr::mutate(icd_code_category = ifelse(concept_code %in% icdCodes$concept_code, 
                ...
                ICD10_Code = tidyr::replace_na(concept_code, 'non_bacterial'),
