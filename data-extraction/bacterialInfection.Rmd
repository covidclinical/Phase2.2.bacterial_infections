...
demo_raw <- files[["patientSummary"]] %>% 
  filter(cohort == "AllAdm") %>%
  arrange(patient_num, as.Date(admission_date), days_since_admission)
obs_raw <- files[["patientObservations"]] %>% 
  filter(cohort == "AllAdm") %>%
  arrange(patient_num, days_since_admission)
clinical_raw <- files[["patientClinicalCourse"]] %>% 
  filter(cohort == "AllAdm") %>%
  arrange(patient_num, as.Date(calendar_date), days_since_admission)
...
### Save the ICD code frequencies for this cohort
```{r}
cohort_icd_codes <- all_codes %>%
  dplyr::select(hospitalization_id, concept_code) %>%
  unique() %>%
  group_by(concept_code) %>%
  summarise(n_hospitalizations = length(hospitalization_id)) %>%
  dplyr::arrange(desc(n_hospitalizations))

save(cohort_icd_codes, file = paste0(dir.output, 'cohort_icd_codes.Rdata'))
...
bacteria_infection_codes_qc <- all_codes %>% 
...
Total number of patients, matching the inclusion
...
```{r}
count_icd_total <- all_codes_desc %>%
  ...
  dplyr::summarise(count_total =  ifelse(n_distinct(hospitalization_id) > obfuscation | isFALSE(obfuscation),
                               ...
                               0.5)) %>%
 dplyr::mutate( period = ifelse( time_p < pandemic_start_date,
                           "before_pandemic",
                           "during_pandemic"))
source("../R/admission_plot_count.R")
nh <- admission_plot_count(df = count_icd_total, 
                     x = "time_p",
                     y = "count_total", 
...
## Number of total hospitalizations with bacterial infection disorders


```{r}
count_icd_bacterial <- all_codes_desc %>%
...
count_icd_perc <- count_icd_total %>%
  dplyr::left_join(count_icd_bacterial, 
            by = c("time_p", "period")) %>% 
  tidyr::replace_na(list(count_bacterial = 0)) %>%
  dplyr::mutate( percentage_bacterial = round(100*count_bacterial/count_total, 3))


thpd_bacterialCount <- count_icd_perc %>%
  pivot_longer(names_to = "counts", cols=c(count_bacterial, count_total)) %>%
...
```{r}
bacterialPercentage <- admission_plot_count(df = count_icd_perc, 
                     x = "time_p",
...
patients_with_bacterial_conditions <- unique( all_codes_desc %>% filter( icd_code_category == "bacterial") %>% select( patient_num, time_p))
...
count_icd_perc <- count_icd_perc %>%
  dplyr::mutate( count_no_bacterial = count_total - count_bacterial )

hospitalization_count_bacterial <- count_icd_perc %>%
...
```{r, its count based}
ts_linear_count <- lm(count_bacterial ~ period * time, hospitalization_count_bacterial)
ts_seasonal_count <- lm(count_bacterial ~ period*time + harmonic(month_year, 1, 12), hospitalization_count_bacterial)
```

```{r, its percentage based}
ts_linear_percent <- lm(percentage_bacterial ~ period * time, hospitalization_count_bacterial)
ts_seasonal_percent <- lm(percentage_bacterial ~ period*time + harmonic(month_year, 1, 12), hospitalization_count_bacterial)
```


### ITS: number of hospitazations, no clearance period, with seasonality effect

```{r}
source("../R/time_series_analysis_plotting.R")

time_series_analysis_plotting(ts_seasonal_count,
                              hospitalization_count_bacterial,
                              "count_bacterial",
...
```{r}
count_disorder_group <- all_codes_desc %>%
  dplyr::filter( icd_code_category == "bacterial", 
          time_p < end_date_plots &
         time_p >= start_date_plots ) %>%
  dplyr::group_by( time_p, disorder_group ) %>%
  summarise(count_dg =  ifelse(n_distinct(hospitalization_id) > obfuscation | isFALSE(obfuscation),
                               n_distinct(hospitalization_id), 
                               0.5)) %>%
  dplyr::mutate( period = ifelse( time_p < pandemic_start_date,
                           "before_pandemic",
...
count_icd_all <- all_codes_desc %>%
  dplyr::filter( icd_code_category == "bacterial", 
         ...
         time_p >= start_date_plots ) %>%
    group_by( time_p ) %>%
  summarise(count_icd =  ifelse(n_distinct(hospitalization_id) > obfuscation | isFALSE(obfuscation),
                               n_distinct(hospitalization_id), 
                               0.5)) %>%
  mutate( period = ifelse( time_p < pandemic_start_date,
                           "before_pandemic",
...
perc_disorder_group <- count_icd_all %>%
  ...
  dplyr::mutate( percentage_dg = round(100*count_dg/count_icd, 3))
...
hospitalizations_agg_sex <- all_codes_desc %>%
  dplyr::filter( icd_code_category == "bacterial", 
          time_p < end_date_plots &
         time_p >= start_date_plots, 
         sex %in% c("female", "male") ) %>%
    group_by( time_p, sex ) %>%
  summarise(count_sex =  ifelse(n_distinct(hospitalization_id) > obfuscation | isFALSE(obfuscation),
                               n_distinct(hospitalization_id), 
                               0.5))

hospitalizations_agg_sex %>%
  ggplot(aes(x = time_p, y = count_sex, fill = sex)) +
  geom_bar(stat="identity", position = "dodge")+
  scale_fill_manual(values = cbPalette) +
  labs(y = "Counts",
       x = time_period,
       title = paste0("Hospitalization counts per ", time_period, " aggregated by sex"),
       subtitle = "Per Sex") + 
...
```{r}
##percentage
total_hospitalizations_bacterial <- all_codes_desc %>%
   dplyr::filter( icd_code_category == "bacterial", 
          time_p < end_date_plots &
         time_p >= start_date_plots) %>%
    group_by( time_p ) %>%
  summarise(count =  ifelse(n_distinct(hospitalization_id) > obfuscation | isFALSE(obfuscation),
                               n_distinct(hospitalization_id), 
                               0.5))

hospitalized_agg_sex_perc <- total_hospitalizations_bacterial %>%
  dplyr::left_join( hospitalizations_agg_sex, by = c("time_p")) %>% 
  tidyr::replace_na(list(count_sex = 0)) %>%
  dplyr::mutate( percentage = round(100*count_sex/count ,3))

hospitalized_agg_sex_perc %>%
...
## Bacterial infection conditions by age group
...
```{r}
# counts per time period and sex
hospitalizations_agg_age <- all_codes_desc %>%
  dplyr::filter( icd_code_category == "bacterial", 
          time_p < end_date_plots &
         time_p >= start_date_plots ) %>%
    group_by( time_p, age_group ) %>%
  summarise(count_age =  ifelse(n_distinct(hospitalization_id) > obfuscation | isFALSE(obfuscation),
                               n_distinct(hospitalization_id), 
                               0.5)) %>%
  dplyr::mutate( period = ifelse( time_p < pandemic_start_date,
                           "before_pandemic",
...
hospitalizations_agg_age %>%  
  ggplot(aes(x = time_p, y= count_age, fill = period, group = age_group )) +
  geom_bar(stat = "identity")+
  geom_vline(xintercept = as.Date(pandemic_start_date),
             linetype = "dashed") +
  geom_smooth(method = "lm", formula = y ~ splines::ns(x, 2), se= TRUE, aes(colour=period), colour= "black", size = 0.5) + 
