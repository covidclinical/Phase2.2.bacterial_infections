...
clinical_filtered  <- clinical_raw %>%
  ...
  dplyr::mutate( date = as.Date( calendar_date, format = dateFormat),
                 ...
                 year = format( date, "%Y"), 
                 period = ifelse( date <= pre_NPI,
                           "pre-NPI", ifelse( date > full_NPI, "partial-NPI", "full-NPI")))
...
bacterialCodesSummary <-bacteria_infection_codes_qc %>%
...
# save the file for QC
save( bacterialCodesSummary, file = paste0(dir.output, 'bacterialCodesSummary.Rdata')) 
```
...
## Number of total monthly hospitalizations
...
count_icd_perc <- monthly_hospitalizations %>%
...
patients_without_bacterial_conditions_counts <- patients_without_bacterial_conditions %>%
  ...
  summarise(count_no_bacterial_patients =  ifelse(n_distinct(patient_num) > obfuscation | isFALSE(obfuscation),
                               ...
                               0.5)) %>%
  mutate( period = ifelse( time_p <= pre_NPI,
                           "pre-NPI", ifelse( time_p > full_NPI, "partial-NPI", "full-NPI")))
