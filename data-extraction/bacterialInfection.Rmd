...
hospitalisations_seq_df <- clinical_raw %>%
  dplyr::distinct(patient_num, cohort, days_since_admission, in_hospital) %>%
  dplyr::group_by(patient_num, cohort) %>%
  dplyr::group_modify(count_sequences_hospitalisation)
...
clinical_filtered <- clinical_raw %>%
  dplyr::filter( len_hospitalisation >= min_hosp_days, 
          cohort  %in% c("AllAdm")) %>%
...
clinical_table <- clinical_filtered %>% 
  dplyr::inner_join( distinct(obs_filtered_age, patient_num, days_since_admission),  
                        by = c("patient_num", "days_since_admission")) %>% 
  dplyr::distinct(patient_num, n_hospitalisation) %>% 
  dplyr::inner_join(clinical_filtered, by = c("patient_num", "n_hospitalisation"))
...
clinical_test <- clinical_table[, c("patient_num", "n_hospitalisation", "days_since_admission")] %>% 
  dplyr::inner_join(distinct(observation_table_ado, patient_num, days_since_admission, age_time_diagnosis), 
            by = c("patient_num", "days_since_admission")) %>% 
  dplyr::distinct(patient_num, n_hospitalisation, age_time_diagnosis)
...
new_hospitalizations <- clinical_table %>%
  dplyr::group_by(patient_num, n_hospitalisation) %>%
...
## New admissions by age group
```{r}
new_hospitalizations_ado_sg <- clinical_table_ado %>%
  dplyr::left_join( ageGroups, by = c( "age_time_diagnosis" = "age")) %>%
  dplyr::group_by(patient_num, n_hospitalisation) %>%
  dplyr::slice(which.min(time_p)) %>%
  dplyr::filter( in_hospital == 1 ) %>%
  dplyr::group_by( time_p, age_group ) %>%
  dplyr::summarise(count = ifelse(n_distinct(patient_num) > obfuscation | isFALSE(obfuscation),
                               n_distinct(patient_num), 
                               0.5)) %>% 
  dplyr::ungroup()
...
admission_plot_count(df = new_hospitalizations_ado_sg, 
                     x = "time_p",
                     y = "count", 
                     title = paste0("New admissions (per ", time_period, ")"), 
                     y_title = "Patient Count")+
...
## Total patients hospitalized per month and age group
```{r}
unique_patients_hospitalized_ag <- clinical_table_ado %>%
  dplyr::left_join( ageGroups, by = c( "age_time_diagnosis" = "age")) %>%
  dplyr::filter(date < end_date_plots &
         date >= start_date_plots ) %>%
  dplyr::group_by( time_p, age_group ) %>%
  dplyr::summarise(count = ifelse(n_distinct(patient_num) > obfuscation | isFALSE(obfuscation),
                               n_distinct(patient_num), 
...
admission_plot_count(df = unique_patients_hospitalized_ag, 
                     x = "time_p",
                     y = "count", 
                     title = paste0("Total hospitalization (per ", time_period, ")"), 
                     y_title = "Patient Count") +
