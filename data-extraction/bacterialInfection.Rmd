...
#obfuscation = 3
dateFormat <- "%Y-%m-%d"
data_update_date <- "2023-01-07"
dir.output <- "/4ceData/bacterialInfection_outputs/dev"
...
if(filecheck){
  ...
  demo_raw <- demo_raw %>% 
    ...
    dplyr::filter(cohort == "AllAdm", 
                  age >= 0,
                  admission_date >= ymd('2019-01-01'),
                  admission_date <= ymd('2023-03-31')) %>%
  ...
  clinical_raw <- clinical_raw %>% 
    dplyr::filter(cohort == "AllAdm",
                  calendar_date >= ymd('2019-01-01'),
                  calendar_date <= ymd('2023-03-31')) %>%
