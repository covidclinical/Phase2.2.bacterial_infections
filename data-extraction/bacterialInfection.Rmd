...
```{r}
#identify the patients with at least one of the disorders
#and estimate if they had 1 or both
pneumonia_empyema <-  all_codes_desc %>% 
  dplyr::filter( disorder_group %in% c("bacterial pneumonia", "LRTI/empyema")) %>%
  dplyr::select( hospitalization_id, disorder_group ) %>%
  unique() 

cases <- pneumonia_empyema %>%
  dplyr::group_by(hospitalization_id) %>%
  dplyr::summarise( n = n_distinct(disorder_group)) %>%
  dplyr::filter( n > 1)

# create a data frame with the rows to exclude
# create an id with the hospitalization id and the disorder group to make it easier to exclude
rows_to_exclude <- pneumonia_empyema %>% 
  dplyr::filter( hospitalization_id %in% cases$hospitalization_id & disorder_group != "LRTI/empyema") %>%
  dplyr::mutate( key = paste0( hospitalization_id , "*", disorder_group))

# remove the rows
alba_method <- all_codes_desc %>%
  dplyr::mutate( key = paste0( hospitalization_id , "*", disorder_group)) %>%
  dplyr::filter( ! key %in% rows_to_exclude$key ) %>%
  dplyr::select( - key )

########
sim_method <- all_codes_desc %>%
  dplyr::group_by(hospitalization_id) %>%
  dplyr::mutate(has_both = "LRTI/empyema" %in% disorder_group & "bacterial pneumonia" %in% disorder_group) %>%
  dplyr::filter(!(disorder_group == "bacterial pneumonia" & has_both)) %>% # remove the bacterial pneumonia rows if the hospitalization has both 
  dplyr::select(-has_both)

diff <- setdiff(alba_method, sim_method)
# there are 4182 rows in sim_method that are not present in alba_method


# test cases
test_cases <- pneumonia_empyema %>% group_by(hospitalization_id) %>% mutate(has_both = "LRTI/empyema" %in% disorder_group & "bacterial pneumonia" %in% disorder_group)

alba_method %>% 
  filter(hospitalization_id == '100594-1',
         !is.na(disorder_group))

sim_method %>% 
  filter(hospitalization_id == '100594-1',
         !is.na(disorder_group))

testdf %>% group_by(hid) %>% mutate(has_both = 'e' %in% code & 'b' %in% code) %>% filter(!(code == 'b' & has_both))
