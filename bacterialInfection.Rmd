...
5.  "time_period": Time unit for the different graphs of the plots:
    months (default), weeks, days

```{r message=FALSE, warning=FALSE}
folder_4ce_files <- "/4ceData/Input/Phase2_AllAdm_March2022/"
obfuscation =  FALSE
#obfuscation = 3
dateFormat <- "%d-%b-%y"
min_hosp_days <- 1 
...
icdCodes <- read.csv("public-data/ICD_bacterial4.csv", header = TRUE, colClasses = "character") %>%
   dplyr::select( disorder_group = syndrome, ICD10_Code, description, bacterial_syndrome )
```
...
### Counting ICD10 bacterial infection codes in the obs_raw table
...
icdCodes$concept_code <- gsub("\\.", "", icdCodes$ICD10_Code) 
...
print( paste0("There are ", length(bacterialInfectionCodes$concept_code), " ICD10 bacterial infection codes out of the total ",length(unique(icdCodes$ICD10_Code)), " in the dataset"))
```
...
-   With a duration of hospitalization spanning over at least two
    consecutive days
...
unit chosen. Not filtered on qge.
...
## Total hospitalize patients age range < 18

Calculating the total number of patients hospitalized on a given day
(taking into account the hospitalization length).
...
### Ratio: patients with bacterial infection condition / patients without bacterial infection condition
...
ratios_patients_with_without_bacterial %>%
  pivot_longer(cols = c("count_no_bacterial_patients", "count_bacterial_patients"), 
               values_to = "count", 
               names_to = "count_type") %>% 
  ...
  labs(y = "Count",
       x = paste0("Date (by ", time_period,")"),
       title = paste0("Count comparison: patients with vs without bacterial infection conditions (per ", time_period,")")) +
  guides(color = guide_legend(title="Patient diagnoses types"))
```

```{r}
ratio_bacterial_non_bacterial <- ratios_patients_with_without_bacterial %>%
  ...
  scale_fill_manual(values = cbPalette) +
  scale_color_manual(values = cbPalette) +
  labs(y = "Ratio (with bacterial conditions/without bacterial conditions)",
       x = paste0("Date (by ", time_period,")"),
       title = paste0("Ratio patient with vs without bacterial infection conditions ( per ", time_period,")"))

ratio_bacterial_non_bacterial
...
```{r}
# Time series analysis of percentage of bacterial codes per encounter, linear (without seasonality)
...
ts_linear_count_clear <- lm(count_bacterial_patients ~ period * time, patient_count_bacterial_period_bacterial_clear)
...
ts_seasonal_count_clear <- lm(count_bacterial_patients ~ period*time + harmonic(month_year, 1, 12), patient_count_bacterial_period_bacterial_clear)
```

```{r, its percentage based}
ts_linear_percent <- lm(percentage ~ period * time, patient_count_bacterial_period_bacterial)
ts_linear_percent_clear <- lm(percentage ~ period * time, patient_count_bacterial_period_bacterial_clear)
ts_seasonal_percent <- lm(percentage ~ period*time + harmonic(month_year, 1, 12), patient_count_bacterial_period_bacterial)
ts_seasonal_percent_clear <- lm(percentage ~ period*time + harmonic(month_year, 1, 12), patient_count_bacterial_period_bacterial_clear)
```
...
time_series_analysis_plotting(ts_linear_count,
  ...
  labs(title = "Interrupted Time-Series Analysis: Count of encounters with bacterial infection ICD codes",
...
### ITS: bacterial percentage, no clearance period, no seasonality effect
...
### ITS: bacterial percentage, clearance period, no seasonality effect
...
### ITS: bacterial percentage, with clearance period, with seasonality effect
...
bootstrapped_timepoints <- mapply(binom_proportion,
                                  n = bootstrap_size,
                                  size = patient_count_bacterial_period_bacterial$count_no_bacterial_patients +
                                    patient_count_bacterial_period_bacterial$count_bacterial_patients,
                                  prob = patient_count_bacterial_period_bacterial$percentage/100)
...
for (iteration in 1:bootstrap_size) {
  df_iteration <- data.frame(period = patient_count_bacterial_period_bacterial$period,
                             time = patient_count_bacterial_period_bacterial$time,
  ...
  df_iteration$time_p <- patient_count_bacterial_period_bacterial$time_p
...
ggplot(bootstrapped_fitted_df, aes(x = time_p, y = predicted, group = interaction(iteration, period))) +
  ...
  geom_point(data = patient_count_bacterial_period_bacterial,
             aes(y = percentage),
             alpha = 0.5) +
  ...
  annotate("text",
           ...
           y = max(patient_count_bacterial_period_bacterial[["percentage"]]) + 5,
           label = plot_annotation) +
  labs(title = "Interrupted Time Series Analysis: Percentage of patients with bacterial infection conditions",
...
```{r, eval=FALSE}
length_hospitalisation_values <- all_codes_bacterial %>% 
...
bacteria_infection_codes_qc infection_output$siteid = site 
count_icd$siteid = site 
patient_count_bacterial_period_bacterial$siteid = site 
patient_count_bacterial_period_bacterial_clear$siteid = site 
...
table1$siteid = site 
ratios_patients_with_without_bacterial$siteid = site 

save( count_icd,
      bacteria_infection_codes_qc infection_output,
      ratios_patients_with_without_bacterial,
      patient_count_bacterial_period_bacterial,
      patient_count_bacterial_period_bacterial_clear,
      ...
      file = paste0("./peds_bacterial_aggResults_", site, ".RData"))
```
